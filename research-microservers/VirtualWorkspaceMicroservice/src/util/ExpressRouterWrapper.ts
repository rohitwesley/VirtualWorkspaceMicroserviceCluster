...
export class ExpressRouterWrapper {
  ...
  public addDeleteHandler (path: string, handler: express.Handler, permissions: string[] = []) {
    this.router.delete(path, this._authFunction(permissions), asyncHandler(handler))
  }

  public addPutHandler (path: string, handler: express.Handler, permissions: string[] = []) {
    this.router.put(path, this._authFunction(permissions), asyncHandler(handler))
  }
  ...
  public addDeleteHandlerNoAuth (path: string, handler: express.Handler) {
    this.router.post(path, asyncHandler(handler))
  }
  
  public addPutHandlerNoAuth (path: string, handler: express.Handler) {
